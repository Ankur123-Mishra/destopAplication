...
export default function IdCardPreview() {
  ...
  const { schoolId, classId, studentId, idCardId } = useParams();
  ...
  if (!card) {
    return (
      <>
        <Header title="ID Card not found" showBack backTo={backTo} />
  ...
  if (isFabric && fabricTemplate?.json) {
    ...
    return (
      <>
        <Header title="ID Card Preview" showBack backTo={backTo} />
        <div className="idcard-preview-page">
          <p className="text-muted" style={{ marginBottom: 20 }}>Saved ID card — click Back to return.</p>
